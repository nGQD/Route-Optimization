...
API_KEY = ""
N = 7
STATES = ["CA", "GB", "PH", "SG", "US"]


def generate_ranking(title: str, values: list) -> None:
    print(f"\n{title}")
    ranking = dict(zip(values, STATES))
...
### PROBLEM 1 ###
POSITIVE, NEGATIVE, STOPWORD = [set(open(f"C:/Users/User/Desktop/WIA2005 Group Project/{file}.txt", encoding="utf-8").read().split(",")) for file in ["POSITIVE", "NEGATIVE", "STOPWORD"]]

articles = [set(open(f"C:/Users/User/Desktop/WIA2005 Group Project/{file}.txt", encoding="utf-8").read().split()) for file in STATES]
...
def plot(array: list, title: str, ylabel: str, colour: str):
    plt.bar(STATES, array, color=colour)
    plt.title(title)
    plt.xlabel("States")
    plt.ylabel(ylabel)
    plt.show()
...
generate_ranking("Ranking of Most Recommended State for Expansion Based on Article Reviews", negativity)
...
df = pd.read_excel(r"C:\Users\User\Desktop\WIA2005 Group Project\WIA2005 csv.xlsx")

costs = []

for state in STATES:

    df_state = df.loc[df["state"] == state]["street_address"]
    stores = df_state.sample(n=N, random_state=42)
    ...
    with open(f"C:/Users/User/Desktop/WIA2005 Group Project/{state}.png", "wb") as img:
        for chunk in plot:
            img.write(chunk)

    plt.imshow(plt.imread(f"C:/Users/User/Desktop/WIA2005 Group Project/{state}.png"))
    plt.show()
    

generate_ranking("Ranking of Most Recommended State for Expansion Based on Traveling Distances", costs)
...
generate_ranking("Final Ranking of Most Recommended State for Expansion", final_ranking)